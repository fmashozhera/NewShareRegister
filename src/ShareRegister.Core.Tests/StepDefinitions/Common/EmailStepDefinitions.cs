...
namespace ShareRegister.Core.Tests.StepDefinitions.Common
{
    ...
    public class EmailStepDefinitions
    {
        ...
        public void ThenIShouldGetAnEmailResultWithValueAndSuccess(string value, string status)
        {
            ...
            Assert.IsTrue(bool.Parse(status) == result.IsSuccess);
        }
        ...
        [Given(@"the valid email address ""([^""]*)""")]
        public void GivenTheValidEmailAddress(string emailAddress)
        {
            _email = emailAddress;
        }

        [Then(@"i should get an success email result with value ""([^""]*)""")]
        public void ThenIShouldGetAnSuccessEmailResultWithValue(string emailAddress)
        {
            Assert.That(emailAddress, Is.EqualTo(result.Value.Value));
            Assert.IsTrue(result.IsSuccess);
        }

        [Given(@"the invalid email address ""([^""]*)""")]
        public void GivenTheInvalidEmailAddress(string emailAddress)
        {
            _email = emailAddress;
        }

        [Then(@"i should get a failure email result with message ""([^""]*)""")]
        public void ThenIShouldGetAFailureEmailResultWithMessage(string failureMessage)
        {
            Assert.IsTrue(result.HasError<InvalidEmailError>());
            Assert.IsTrue(result.Errors.FirstOrDefault(t => t.Message == failureMessage) != null);
